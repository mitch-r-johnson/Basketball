...
def get_historical_data(team_id: str) -> object:
    ...
    clean_df.to_csv(f"/Users/mitchjohnson/PycharmProjects/Basketball/Data/{team_id}-historical.csv")

def get_current_season(team_id):
    games = get_data.transform_game2(get_data.get_current_season_games(team_id))
    games_df = pd.DataFrame(games)

    box_scores = get_data.flatten_games_by_game_id2(get_data.get_current_season_box_scores(team_id))
    box_scores_df = pd.DataFrame(box_scores)
    ...
    clean_df.to_csv(f"/Users/mitchjohnson/PycharmProjects/Basketball/Data/{team_id}-current.csv")

for team_id in range(132,162):
    get_historical_data(team_id)
    get_current_season(team_id)
